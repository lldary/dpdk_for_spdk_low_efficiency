					  ...
					  struct pci_msix_table *msix_table)
{
	...
	if (cap_offset != 0)
	{
		...
		if (rte_pci_read_config(dev, &reg, sizeof(reg), cap_offset + RTE_PCI_MSIX_TABLE) < 0)
		{
			RTE_LOG(ERR, EAL,
		...
		if (rte_pci_read_config(dev, &flags, sizeof(flags), cap_offset + RTE_PCI_MSIX_FLAGS) < 0)
		{
			RTE_LOG(ERR, EAL,
				  ...
				  int bar_index, int additional_flags)
{
	...
	if (bar_addr != MAP_FAILED)
	{
		...
		if ((map_addr != NULL || memreg[0].size == 0) && memreg[1].offset && memreg[1].size)
		{
