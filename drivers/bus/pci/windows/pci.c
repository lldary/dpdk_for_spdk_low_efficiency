...
bool pci_device_iommu_support_va(const struct rte_pci_device *dev __rte_unused)
{
	/* This function is not implemented on Windows.
	 * We really should short-circuit the call to these functions by
	 * clearing the RTE_PCI_DRV_NEED_MAPPING flag
...
parse_pci_hardware_id(const char *buf, struct rte_pci_id *pci_id)
{
	...
	ids = sscanf_s(buf, "PCI\\VEN_%" PRIx16 "&DEV_%" PRIx16 "&SUBSYS_%" PRIx32, &vendor_id, &device_id, &subvendor_id);
	if (ids != 3)
		return -1;
...
pci_scan_one(HDEVINFO dev_info, PSP_DEVINFO_DATA device_info_data)
{
	...
	/* get resources */
	if (get_device_resource_info(dev_info, device_info_data, dev) != ERROR_SUCCESS)
	{
		goto end;
	}
