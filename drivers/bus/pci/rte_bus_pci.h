...
#include <rte_pci.h>
	 ...
	 *    true/false
	 */
	__rte_experimental bool rte_pci_has_capability_list(const struct rte_pci_device *dev);
	...
	__rte_experimental
		off_t
		rte_pci_find_next_capability(const struct rte_pci_device *dev, uint8_t cap, off_t offset);
	 ...
	 */
	__rte_experimental int rte_pci_set_bus_master(const struct rte_pci_device *dev, bool enable);
	 ...
	 *   0 on success, -1 on error in PCI config space read/write.
	 */
	__rte_internal int rte_pci_pasid_set_state(const struct rte_pci_device *dev,
	 ...
	 *   Number of bytes read on success, negative on error.
	 */
	__rte_experimental int rte_pci_mmio_read(const struct rte_pci_device *device, int bar,
											 void *buf, size_t len, off_t offset);
	 ...
	 */
	__rte_experimental int rte_pci_mmio_write(const struct rte_pci_device *device, int bar,
											  const void *buf, size_t len, off_t offset);
